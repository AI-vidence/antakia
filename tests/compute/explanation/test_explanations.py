...
from tests.utils_fct import test_progress_bar
...
def test_compute_explanations():
    ...
    for i in range(4):
        if i in (1, 2):
            X_exp = compute_explanations(X, model_DT, i, test_progress_bar.update)
            pd.testing.assert_index_equal(X_exp.index, X.index)
            pd.testing.assert_index_equal(X_exp.columns, X.columns)
        else:
            with pytest.raises(ValueError):
                compute_explanations(X, model_DT, i, test_progress_bar.update)
