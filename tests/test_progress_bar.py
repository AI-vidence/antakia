...
class TestMultiStepProgressBar(unittest.TestCase):
    ...
    def test_init_MSPB():
        X, y, _, exp_val, function = generate_ExplanationValues('DT')
        mspb = MultiStepProgressBar(exp_val.get_progress_bar(), steps=20)
        assert mspb.steps == 20
        # assert mspb.widget == exp_val.get_progress_bar()
