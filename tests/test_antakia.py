...
class TestAntakia(TestCase):
    ...
    def test_run_walk(self):
        atk = AntakIA(self.X, self.y, self.regression_DT)
        walk = [('set_proj_method', [1, 1]), ('change_tab', [1]),
                ('set_color', [2]),
                ('select_points', [1]), ('select_dim', [1]), ('unselect', [1]),
                ('edit_parameter', [1]), ('unselect', [0]),
                ('select_dim', [1]), ('change_tab', [2]),
                ('set_proj_method', [1, 2]), ('change_tab', [0]),
                ('select_points', [0]), ('find_rules', []),
                ('set_exp_method', [2])]
