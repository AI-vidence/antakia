...
class TestExplanationValues(AntakiaTestCase):
    ...
    def test_init(self):  # ajouter test click
        ...
        exp_val = ExplanationValues(self.data_store, self.on_change_callback,
                                    self.callable)
        ...
        np.testing.assert_array_equal(exp_val.explanations['Imported'],
                                      self.data_store_w_exp.X_exp)
    ...
    def test_initialize(self, cpt_exp):
        ...
        exp_val1 = ExplanationValues(self.data_store, self.on_change_callback,
                                     self.callable)
    ...
    def test_disable_selection(self):
        exp0 = ExplanationValues(self.data_store, self.on_change_callback,
                                 self.callable)
        exp0.disable_selection(True)
        assert exp0.explanation_select.disabled

        exp2 = ExplanationValues(self.data_store, self.on_change_callback,
                                 self.callable)
