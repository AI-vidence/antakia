...
class TestRulesWidget(AntakiaTestCase):
    ...
    def test_rule_widget(self):
        ...
        rule1 = Rule(var, max=10,
                     includes_max=False)  # None, None, var, '<', 10)
        rule2 = Rule(var, min=10,
                     includes_min=True)  # 10, '<=', var, None, None)
        rule3 = Rule(var,
                     min=10,
                     includes_min=True,
                     max=40,
                     includes_max=False)  # 10, '<=', var, '<', 40)
        rule4 = Rule(var2, min=10,
                     includes_min=True)  # 10, '<=', var2, None, None)
        ...
        rw = RuleWidget(rule1, self.data_store, self.data_store.X, True,
