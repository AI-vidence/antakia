...
class ProjectedValues:
    ...
    def set_parameters(self, projection_method, dimension, kwargs):
        ...
        if self._kwargs.get(Proj(projection_method, dimension)) is None:
            self.build_default_parameters(projection_method, dimension)

        self._kwargs[Proj(projection_method, dimension)]['previous'] = self._kwargs[Proj(projection_method, dimension)][
            'current'].copy()
        self._kwargs[Proj(projection_method, dimension)]['current'].update(kwargs)
        del self._projected_values[Proj(projection_method, dimension)]

    def get_parameters(self, projection_method, dimension):
        if self._kwargs.get(Proj(projection_method, dimension)) is None:
            self.build_default_parameters(projection_method, dimension)
        return self._kwargs.get(Proj(projection_method, dimension))
    ...
    def build_default_parameters(self, projection_method, dimension):
        ...
        self._kwargs[Proj(projection_method, dimension)] = {
            'current': current,
