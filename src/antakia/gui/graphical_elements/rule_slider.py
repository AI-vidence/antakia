...
class RuleSlider:
    ...
    def build_widget(self):
        ...
        self.widget = v.Row(
            children=[self.min_input, self.range_slider, self.max_input], align="center"
        )
    ...
    def _update_from_slider(self, *args):
        if self.value[0] is not None and self.value[0] != self.range_slider.v_model[0]:
            self._update_min_slider()
        elif self.value[0] is None and self.range_slider.v_model[0] >= self.range[0]:
            self._update_min_slider()
        if self.value[1] is not None and self.value[1] != self.range_slider.v_model[1]:
            self._update_max_slider()
        elif self.value[1] is None and self.range_slider.v_model[1] <= self.range[1]:
            self._update_max_slider()
        self._display()
        self.change_callback(self.value)
