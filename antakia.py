...
# le nom des 3 modèles proposées pour les sub_models
...
def start(
    ...
    def changement(widget, event, data):
    ...
    slider_clusters = v.Slider(
        style_="width : 30%", class_="ma-3 mb-0", min=2, max=20, step=1, v_model=5
    )
    ...
    texte_launch = widgets.HTML(value="<h5> Invisible normalement... </h5>")
    texte_launch.layout.display = "none"

    bouton_launch = widgets.Button(
        description="Launch",
        disabled=False,
        button_style="",
        icon="paper-plane-o",
    ...
    def fonction_launch(b):
        bouton_launch.button_style = "success"
        texte_launch.value = (
            "Déploiement (" + str(len(liste_tuiles)) + " région(s) à envoyer)"
        )
        texte_launch.layout.display = "block"
        launch()
        time.sleep(4)
        texte_launch.layout.display = "none"
        bouton_launch.button_style = ""
...
def launch():
    ...
    if len(liste_tuiles) == 0:
        ...
        Y_entier.to_csv("data_transfer/y_0.csv", index=False)
        SHAP_entier.to_csv("data_transfer/SHAP_0.csv")
        for i in range(len(liste_tuiles)):
            ensemble[i]["X"].to_csv("data_transfer/X_{}.csv".format(i + 1))
            ensemble[i]["y"].to_csv("data_transfer/y_{}.csv".format(i + 1), index=False)
            ensemble[i]["SHAP"].to_csv(
                "data_transfer/SHAP_{}.csv".format(i + 1),
            )
            pd.DataFrame(ensemble[i]["indices"]).to_csv(
                "data_transfer/indices_{}.csv".format(i + 1), index=False
            )
    enablePrint()


def len1():
    return len(liste_tuiles)
